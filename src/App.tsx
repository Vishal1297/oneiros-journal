    ...
    <header className="flex items-center justify-between px-8 py-5 bg-brand-surface/80 backdrop-blur-xl sticky top-0 z-50 border-b border-brand-border">
      ...
      <div className="flex items-center gap-6">
        <div className="hidden sm:flex flex-col items-end">
          ...
          <span className="text-[10px] text-slate-500 tracking-wider font-mono">{user.email}</span>
        </div>
        <button 
          onClick={onLogout}
...
function DreamRecorder({ onComplete }: { onComplete: (dream: { transcription: string; analysis: any; imageUrl: string }) => void }) {
  ...
  const processAudio = async (blob: Blob) => {
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        const transcription = await geminiService.transcribeAudio(base64Audio, blob.type);
  ...
  if (isProcessing) {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center">
        ...
        <h3 className="text-base font-semibold text-white mb-1 font-display uppercase tracking-widest">Processing</h3>
        <p className="text-xs text-indigo-300 animate-pulse uppercase tracking-[0.2em]">{processingStatus}</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col items-center gap-8 p-12">
      ...
      <div className="text-center">
        ...
        <h3 className="text-sm text-slate-300 font-light italic max-w-[200px]">
          {isRecording ? "Transcribing your subconscious..." : "Narrate your journey for aethersphere mapping"}
        </h3>
      </div>
    </div>
  );
...
function DreamChat({ dream, onUpdate }: { dream: DreamEntry; onUpdate: (updatedDream: DreamEntry) => void }) {
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const finalHistory = [...newHistory, { role: 'model' as const, text: response }];
    ...
    <div className="flex flex-col h-full bg-brand-surface rounded-3xl border border-brand-border overflow-hidden">
      <div className="p-5 border-b border-brand-border bg-white/5 backdrop-blur-xl flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-bold tracking-[0.2em] text-indigo-400 uppercase">Symbol Chat</span>
        </div>
      </div>

      <div ref={scrollRef} className="flex-1 overflow-y-auto p-5 space-y-4 font-sans no-scrollbar">
      ...
      <form onSubmit={sendMessage} className="p-4 bg-white/5 flex gap-2 border-t border-brand-border backdrop-blur-md">
        <input
          type="text"
          value={message}
...
function DreamView({ dream, onBack, onDelete }: { dream: DreamEntry; onBack: () => void; onDelete: (id: string) => void }) {
    ...
    >
      <div className="flex items-center justify-between mb-8">
        ...
        >
          ...
          <span className="text-[10px] font-bold tracking-[0.2em] uppercase">Return to Aethersphere</span>
        </button>
        <button 
          onClick={() => {
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-stretch">
        ...
        <div className="lg:w-1/4 flex flex-col gap-6">
          <div className="p-6 rounded-3xl bg-white/5 border border-brand-border flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-bold tracking-widest text-indigo-400 uppercase">Voice Capture</span>
          ...
          <div className="flex-1 p-6 rounded-3xl bg-white/5 border border-brand-border">
            <span className="text-[10px] font-bold tracking-widest text-indigo-400 uppercase block mb-4">Archetypal Map</span>
        ...
        <div className="flex-1 flex flex-col gap-6">
          ...
          <div className="p-6 rounded-3xl bg-indigo-950/20 border border-indigo-500/20 flex items-center gap-6">
            <div className="w-12 h-12 rounded-2xl bg-indigo-500/20 border border-indigo-400/40 flex items-center justify-center text-xl shrink-0">✨</div>
            <div>
              <p className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest mb-1">Subconscious Insight</p>
              <p className="text-sm text-indigo-100/70 leading-relaxed font-light italic">
                The visual representation highlights the fluidity of your memory during this specific REM cycle.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="lg:w-1/4 flex flex-col gap-6">
          <div className="p-6 rounded-3xl bg-white/5 border border-brand-border h-[400px] overflow-y-auto custom-scrollbar">
            <div className="flex items-center gap-2 mb-6">
              ...
              <span className="text-[10px] font-bold tracking-[0.2em] text-indigo-400 uppercase">Analysis</span>
            </div>
...
function DreamGallery({ dreams, onSelect }: { dreams: DreamEntry[]; onSelect: (dream: DreamEntry) => void }) {
  if (dreams.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-16 text-center bg-white/5 rounded-[40px] border border-dashed border-brand-border">
        ...
        <h3 className="text-lg font-medium text-slate-300 font-display">The Aethersphere is still.</h3>
          ...
          <div className="aspect-[4/5] bg-slate-900 relative">
            ...
            <div className="absolute bottom-6 left-6 z-20 pr-6">
              <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1 group-hover:text-indigo-400 transition-colors">
                {dream.timestamp?.toDate ? format(dream.timestamp.toDate(), 'MMM dd • yyyy') : 'Recently captured'}
              </p>
...
export default function App() {
  ...
  const saveDream = async (data: { transcription: string; analysis: any; imageUrl: string }) => {
    if (!user) return;
          ...
          {selectedDream ? (
            ...
            >
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-brand-border pb-10">
                <div>
                  <span className="text-[10px] font-bold tracking-[0.4em] text-indigo-400 uppercase block mb-3">Subconscious Archive</span>
                  <h2 className="text-4xl font-bold font-display text-white tracking-tight">Galleries of Night</h2>
                  <p className="text-slate-500 mt-2 font-light">Documented subconscious projection clusters.</p>
                </div>
                <button
              ...
              <DreamGallery dreams={dreams} onSelect={(d) => setSelectedDreamId(d.id)} />
            </motion.div>
          )}
        </AnimatePresence>
      </main>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            >
              <div className="p-6 border-b border-brand-border flex justify-between items-center bg-white/5">
                <span className="text-[10px] font-bold tracking-[0.2em] text-indigo-400 uppercase">Capture Sequence</span>
                <button 
                  onClick={() => setIsRecordingModalOpen(false)}
